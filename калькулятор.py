...
def noth(e):
    ...
    pass
def refill():
    if len(ol['text']) > 2 and ol['text'][-2:] == '.0': ol['text'] = ol['text'][:-2]
    if '.' not in ol['text']:
        if len(ol['text']) > 15:
            removeall()
        ...
        if len(ol['text']) > 15:
            if ol['text'].index('.') <= 14:
                ...
                removeall()
                ol['text'] = 'Переполнение'                   
def remove(e=''): ol['text'] = ol['text'][:len(ol['text'])-1] if len(ol['text']) > 1 else '0'
def removetext(e=''): ol['text'], curval = '0', ''
def removeall():
    global memory, IHBC, SHBD, curval, SD
    ol['text'], l['text'], memory, curval, do, IHBC, SHBD, ml['text'], SD = '0', '', '0', '', '', False, False, '', False
def doing(x):
    global SHBD, IHBC, curval, do, SD
    if IHBC:
        if not SD: l['text'] += ol['text'] + ' ' + x + ' '
        else: l['text'] += x + ' '
        if ol['text'] == '0' and do == '/':
            removeall()
            ol['text'] = 'Деление на ноль'
        if curval: ol['text'] = str(float(curval) + float(ol['text'])) if do == '+' else str(float(curval) - float(ol['text'])) if do == '-' else str(float(curval) * float(ol['text'])) if do == '*' else str(float(curval) / float(ol['text']))  if do =='/' else ol['text']
        refill()
        IHBC, SHBD, curval, SD =  False, True, ol['text'] if ol['text'] != 'Переполнение' else '', False
    else: l['text'] = l['text'] + x + ' ' if l['text'][-2] == ')' else l['text'][:-2] + x + ' '
def wrinm():
    global memory
...
def memoryplus():
    ...
    if len(memory) > 2 and memory[-2:] == '.0': memory = memory[:-2]
def delm():
    global memory
    memory, ml['text'] = '0', ''
def plus(e=''):
...
def dot(e=''):
    global SHBD, IHBC
    ol['text'], IHBC, SHBD = ol['text'] + '.' if len(ol['text']) < 14 and not SHBD else '0.' if ol['text'] == '0' or SHBD else ol['text'], True, False
def enter(e=''):
    ...
    if IHBC:
        ...
        if ol['text'] == '0' and do == '/':
            removeall()
            ol['text'] = 'Деление на ноль'
        if curval: ol['text'] = str(float(curval) + float(ol['text'])) if do == '+' else str(float(curval) - float(ol['text'])) if do == '-' else str(float(curval) * float(ol['text'])) if do == '*' else str(float(curval) / float(ol['text']))
        refill()              
        IHBC, SHBD, curval, do =  True, True, '', ''      
def negate(e=''):
    if ol['text'] != '0': ol['text'] = str(-float(ol['text']))
    if len(ol['text']) > 2 and ol['text'][-2:] == '.0': ol['text'] = ol['text'][:-2]
def place0(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '0' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '0' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
        ol['text'], SD, IHBC, SHBD = '0', False, True, False
        i = len(l['text']) - 1
        while i != 0 and l['text'][i] != '+' and l['text'][i] != '-' and l['text'][i] != '*' and l['text'][i] != '/':
            l['text'], i = l['text'][:i], i - 1
        l['text'] += ' '
def place1(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '1' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '1' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place2(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '2' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '2' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place3(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '3' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '3' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place4(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '4' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '4' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place5(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '5' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '5' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place6(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '6' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '6' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place7(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '7' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '7' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place8(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '8' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '8' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
def place9(e=''):
    global SHBD, IHBC, SD
    if not SD: ol['text'], IHBC, SHBD = ol['text'] + '9' if ol['text'] != '0' and len(ol['text']) < 14 and not SHBD else '9' if ol['text'] == '0' or SHBD else ol['text'], True, False
    else:
...
memory, do = '0', ''
ol = Label(main, anchor='e', fg='blue', font='arial 24', height=1, width=15, relief='sunken', text='0')
l = Label(main, anchor='e', font='arial 10', height=1, width=36, relief='sunken')
ml = Label(main, font= 'arial 25', height=1, width=1) 
ol.place(x=10, y=5)
l.place(x=10, y=50)
ml.place(x=271, y=75)
buttons = [Button(main, text='Backspace', height=2, width=10, command=remove),
